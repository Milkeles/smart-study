...
TranslateFilter::TranslateFilter(QWidget *w, QPoint offset, int durationMs,
                                 QEasingCurve::Type easingType)
    : QObject(w), target(w), offset(offset), duration(durationMs),
      easing(easingType) {}
...
void GuiUtils::addShadowEffect(QWidget *widget, int blur, QPointF offset,
                               QColor color) {
  if (!widget)
    return;
...
void GuiUtils::addTranslateEffect(QWidget *widget, QPoint offset, int duration,
                                  QEasingCurve::Type easing) {
  if (!widget)
    return;

  widget->installEventFilter(
      new TranslateFilter(widget, offset, duration, easing));
